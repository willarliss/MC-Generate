...
def imbalanced(X_train, X_test, y_train, y_test):
    ...
    print('Imbalanced', accuracy_score(y_test, preds)*100, roc_auc_score(y_test, preds), recall_score(y_test, preds))
...
def markov_upsampling(X_train, X_test, y_train, y_test):
    ...
    # print(np.min([len(doc.split()) for doc in minority]))
    
    mcu = MCGen()
    mcu.build(minority)
    synthetic = mcu.generate(len(X_train)-(2*len(minority)))
    # print(np.min([len(doc.split()) for doc in synthetic]))
    ...
    clf.fit(X_train, y_train)
    preds = clf.predict(X_test)
    print('MCUpsampling', accuracy_score(y_test, preds)*100, roc_auc_score(y_test, preds), recall_score(y_test, preds))
...
def bootstrap_upsampling(X_train, X_test, y_train, y_test):
    ...
    clf.fit(X_train, y_train)
    preds = clf.predict(X_test)
    print('Bootstrap up', accuracy_score(y_test, preds)*100, roc_auc_score(y_test, preds), recall_score(y_test, preds))
...
def bootstrap_downsampling(X_train, X_test, y_train, y_test):
    ...
    clf.fit(X_train, y_train)
    preds = clf.predict(X_test)
    print('Bootstrap down', accuracy_score(y_test, preds)*100, roc_auc_score(y_test, preds), recall_score(y_test, preds))
...
def random_bag(X_train, X_test, y_train, y_test):
    ...
    clf.fit(X_train, y_train)
    preds = clf.predict(X_test)
    print('Random bag', accuracy_score(y_test, preds)*100, roc_auc_score(y_test, preds), recall_score(y_test, preds))
